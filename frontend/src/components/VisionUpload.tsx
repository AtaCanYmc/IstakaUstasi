...
export const VisionUpload: React.FC = () => {
  ...
  return (
    <div className="w-full rounded-2xl bg-card-bg border border-card-border p-6 backdrop-blur-md shadow-sm">
      ...
      {!token ? (
        <div className="p-6 rounded-xl border border-dashed border-panel-border text-center bg-panel-bg shadow-sm">
          <AlertCircle className="w-8 h-8 text-amber-500 mx-auto mb-3" />
          <h3 className="font-semibold text-sm text-text-primary">{t('authRequired')}</h3>
          <p className="text-xs text-text-secondary max-w-xs mx-auto mt-1 mb-4">
            {t('authRequiredDesc')}
          </p>
          <span className="text-xs text-indigo-500 dark:text-indigo-400 font-semibold">{t('authRequiredAction')}</span>
        </div>
      ) : (
            ...
            <div
              ...
              className={`border-2 border-dashed rounded-xl p-8 text-center cursor-pointer transition-all duration-200 ${
                dragActive
                  ...
                  : 'border-panel-border bg-panel-bg hover:bg-panel-bg/80'
              }`}
            >
              <input
                ref={fileInputRef}
              ...
              <p className="text-xs font-semibold text-text-secondary">{t('dragDropDesc')}</p>
              <p className="text-[10px] text-text-tertiary mt-1">{t('supportFormat')}</p>
            </div>
          ) : (
            <div className="space-y-4">
              <div className="relative rounded-xl overflow-hidden border border-card-border max-h-64 flex justify-center bg-black">
                <img src={previewUrl} alt="Board preview" className="object-contain max-h-64 w-full" />
                <button
                  onClick={clearImage}
                  disabled={isProcessingVision}
                  className="absolute top-2 right-2 px-2 py-1 rounded bg-black/60 hover:bg-black/80 text-white text-[10px] font-bold cursor-pointer"
                >
                  {t('changeImage')}
                </button>
              </div>
              ...
              {visionError && (
                <div className="p-3 rounded-lg bg-rose-500/10 border border-rose-500/20 text-rose-600 dark:text-rose-300 text-xs flex gap-2 items-start">
                  <AlertCircle className="w-4 h-4 flex-shrink-0 mt-0.5" />
                  <span>{t(visionError)}</span>
                </div>
              )}
              ...
              <div className="grid grid-cols-2 gap-3">
                <button
                  ...
                  disabled={isProcessingVision}
                  className="py-2.5 rounded-xl text-xs font-bold bg-btn-sec-bg hover:bg-btn-sec-hover text-btn-sec-text border border-card-border flex items-center justify-center gap-2 cursor-pointer transition-all active:scale-98 disabled:opacity-50"
                >
                  {isProcessingVision ? (
